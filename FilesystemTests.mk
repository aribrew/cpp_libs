...
INCLUDE							:= . FilesystemHelpers Log

# Libs paths to include in libraries search
LIBS_PATHS 						:= Log

# Dynamic libraries (libwhatever.so) to link
LIBS							:= 

# Static libraries (libwhatever.a) to add
STATIC_LIBS						:= liblog.a
...
${EXEC}: ${BUILD_PATH} ${MAIN_OBJ} ${OBJS} ${MORE_OBJS} ${STATIC_LIBS}
ifeq ($(suffix ${MAIN_SRC}),.c)
	${C} ${OBJS} ${MORE_OBJS} ${STATIC_LIBS} ${MAIN_OBJ} \
		 -o $@ ${LINK_FLAGS} ${C_FLAGS}
else
	${CXX} ${OBJS} ${MORE_OBJS} ${STATIC_LIBS} ${MAIN_OBJ} \
		   -o $@ ${LINK_FLAGS} ${CXX_FLAGS}
endif


# Builds the library
${LIBRARY}: ${BUILD_PATH} ${OBJS} ${MORE_OBJS} ${STATIC_LIBS}
